...
def train(config: Dict=None) -> Trainer:
    ...
    maps_img1k = fetch_difumo(dimension=1024).maps
    maps_masker1k = NiftiMapsMasker(
        maps_img=maps_img1k, 
        smoothing_fwhm=3,
        standardize='zscore',
        detrend=True,
        ...
        high_pass=0.008,)
    maps_masker1k.fit()
    np.savetxt(X=d, fname="./data/dictionary.txt")
    d = maps_masker1k.maps_img_.dataobj.reshape(-1, 1024).transpose(1, 0)

    maps_img128 = fetch_difumo(dimension=128).maps
    maps_masker128 = NiftiMapsMasker(
        maps_img=maps_img128, 
        smoothing_fwhm=3,
        standardize='zscore',
        detrend=True,
        verbose=0,
    ...
    d = maps_masker128.maps_img_.dataobj
    list = []
    maxsize = 0
    for i in range(0, d.shape[-1]):
        a = d[:,:,:,i]
        idx = np.nonzero(a)
        # nonzero_values = a[idx]
        if idx[0].size > maxsize:
            maxsize = idx[0].size
        list.append(idx)

    path = '/mnt/sdd/zhoujie/learning-from-brains-master/data/upstream64'
    for ite, q in enumerate(train_dataset):
        print(ite)
        path1 = path + '/'+ q['__key__'][3:11]
        # os.makedirs(path1, exist_ok=True)

        bold = maps_masker1k.inverse_transform(q['bold.pyd']).dataobj
        bold.max()
        np.abs(bold).min()
        pac = []
        for l in list:
            value = bold[l].transpose(1, 0).astype('float16')
            zero = np.zeros([value.shape[0] , maxsize - value.shape[1]], dtype=value.dtype)
            v = np.concatenate([value, zero], axis=1)[None]
        # bold1 = maps_masker.fit_transform(bold)
            pac.append(v)

        data = np.concatenate(pac, axis=0)
